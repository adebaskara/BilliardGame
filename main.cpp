	...
	for (int i = 0; i < holes.size(); i++) {
		if (balls[index].collideToHole(&holes[i])) {
			...
			if(scored.size()==15)
			{
				...
				endGame = true;
				PlaySound("sfx/Applause.wav", NULL, SND_ASYNC|SND_FILENAME|SND_NOSTOP);
			}
			break;
		}
	}
...
void PlayDisplay() {
	...
	if(endGame && playerMode==2)
	{
		...
		glColor3f(1,0,0);
		drawRectangle(window_width/2 - 75, window_height/2 + 20, 150, 75);
		printString(window_width/2 - 52, window_height/2 + 62, kWhite,"MAIN MENU",6);
	}
	else if(endGame && playerMode==1)
	{
		glColor4f(0, 0, 0, 0.4);
		drawRectangle(0,0,window_width,window_height);
		printString(315,250,kYellow,"CONGRATULATION",6);
		printString(320,280,kYellow,"Training is over",6);
	}
...
void ConfDisplay()
{
	...
	if(playerMode==2)
	{
		...
		printString(125,470,kWhite,"2",6);
		printString(260,470,kWhite,"3",6);
		printString(395,470,kWhite,"4",6);
		printString(530,470,kWhite,"5",6);
		printString(665,470,kWhite,"6",6);
	}
...
void Mouse(int button, int state, int x, int y) {
	...
	else if(button == GLUT_LEFT_BUTTON && pageState==1)
	{
		...
		{
			if(state == GLUT_UP){
				if(x>=window_width/2 - 75 && x<=window_width/2 + 75 && y>=window_height/2 + 20 && y<=window_height/2 + 145)
				{
					Init();
					glutDisplayFunc(MainMenuDisplay);
				}
